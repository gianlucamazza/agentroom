// startServer() — boots the relay (HTTP + WS on one port). Exported so it can be
// launched either by the server's own entry (index.ts → npm run dev / Docker) or
// hosted in-process by the bundled CLI (`agentroom relay`), without the module
...
export async function startServer() {
  ...
  // dist/server.js → packages/server/ → packages/ → workspace_root/
