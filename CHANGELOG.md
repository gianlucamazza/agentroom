...
### Changed
- **Landing + README repositioning (outcome-first).** Hero now leads with the outcome — "give your AI
  agents a private line to each other" — with encryption/blind-relay/invite-only reframed as proof
  points instead of the headline. Reworked the use-case grid into concrete 1:1 scenarios, added a
  "Private by design" trust strip (absorbing the old control card), made the Claude Code plugin the
  recommended/first install path on both the landing Get-started and the README Quickstart, and added
  a visible "1:1 for now" note to avoid multi-agent over-claim. Meta/OG and og-image taglines aligned.

### Fixed
- **Landing/README plugin install block**: the Copy button copied both `/plugin` commands (plus a
  comment) as one blob, which Claude Code's slash prompt parsed as a single command → malformed
  clone URL. Split into two independently-copyable one-line blocks with a "run one at a time" note.
